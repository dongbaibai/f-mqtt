...
/**
 * 消费Kafka中事件,便于同步内存数据「广播」
 */
public class KafkaSyncService {

    private final static Logger log = LoggerFactory.getLogger(KafkaSyncService.class);
    // 解决多线程操作KafkaConsumer的情况,ConcurrentModificationException
    private final ReentrantLock lock = new ReentrantLock();
    private final static String GROUP_ID_PREFIX = "event-sync-group-";
    private volatile boolean running = false;

    private KafkaConsumer<String, String> kafkaConsumer;

    public KafkaSyncService() {
        ...
        AssertUtils.isTrue(StringUtils.isNotBlank(BrokerConfig.eventKafkaConsumerSyncSubscribeTopics)
                , BrokerConfig.EVENT_KAFKA_CONSUMER_SYNC_SUBSCRIBE_TOPICS + "MUST be set");
        ...
        // 广播
        props.put("group.id", GROUP_ID_PREFIX + BrokerConfig.serverName);
...
//        props.put("auto.offset.reset", "earliest");
    ...
    public void start(Consumer<Event> eventConsumer) {
        log.info("KafkaConsumeService subscribe topics:[{}]", BrokerConfig.eventKafkaConsumerSyncSubscribeTopics);
        // We can group consumers into groups
        kafkaConsumer.subscribe(Arrays.stream(BrokerConfig.eventKafkaConsumerSyncSubscribeTopics.split(","))
